"""
================================================================================
RUDE / ESCALATION DETECTOR — FASTAPI BACKEND
Serves the model you just downloaded to:
    C:/Users/27yas/Downloads/latest_sentiment_analysis_model

Folder must contain: config.json, model.safetensors, tokenizer*, heads.pt,
threshold.txt (all present per your screenshot).

RUN
--------------------------------------------------------------------------------
1) pip install fastapi uvicorn "transformers>=4.40" torch --break-system-packages
   (use CPU torch build if you don't have a local GPU — that's fine, this is
   a single xlm-roberta-base forward pass, CPU latency is ~50-150ms/request)

2) Set MODEL_DIR (or just edit the default below) and run:
       set MODEL_DIR=C:/Users/27yas/Downloads/latest_sentiment_analysis_model
       uvicorn rude_detector_api:app --host 0.0.0.0 --port 8000

3) Test:
       curl -X POST http://localhost:8000/predict -H "Content-Type: application/json" -d "{\"text\": \"tu pagal hai bilkul bekaar service hai\"}"

NOTES CARRIED OVER FROM YOUR PAST DEBUGGING (so this doesn't repeat them):
  - threshold.txt is read at startup; if missing/unreadable, falls back to 0.5
    and logs a warning instead of silently guessing.
  - All file reads use encoding="utf-8-sig" to handle BOM issues with
    Devanagari text on Windows.
  - MODEL_DIR is resolved to an absolute path and validated (all required
    files checked) before the model is loaded, so path-confusion errors
    show up immediately at startup, not on the first request.
  - Keyword + tone hybrid override layer is kept (same behavior as your
    training script's RudeDetector.predict), since that was deliberately
    added on top of the ML model.
================================================================================
"""
...
# ═════════════════════════════════════════════════════════════════════════
# KEYWORD / TONE LOGIC (same as training script, needed for the hybrid layer)
# ═════════════════════════════════════════════════════════════════════════
